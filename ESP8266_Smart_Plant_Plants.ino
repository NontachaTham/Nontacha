...
int delay_for_one_loop = 10000; //delay 60 sec for 1 loop (auto)
...
void loop() {
  ...
  if (WiFi.status() == WL_CONNECTED) { //Check WiFi connection status
    ...
    http.begin(client, "http://575781d3.ngrok.io/sensordatastatus");
    ...
    if (autostatus != obj["auto"].as<String>()) {
      ...
      http.begin(client, "http://575781d3.ngrok.io/automodereply");     //Specify request destination
    ...
    if (automode == "OFF") {
      if (doc["soil moisture"] < soilmoisture_at_least) { //notification
        http.begin(client, "http://575781d3.ngrok.io/notification");     //Specify request destination
        http.addHeader("Content-Type", "application/json");  //Specify content-type header
      ...
      if (obj["sentdata"].as<String>() == "1") {
      ...
      else if (obj["sentdata"].as<String>() == "0") {
        if (valvestatus != obj["valve"].as<String>()) {
          ...
          http.begin(client, "http://575781d3.ngrok.io/valvereply");     //Specify request destination
          http.addHeader("Content-Type", "application/json");  //Specify content-type header
          valvetext = "Valve is ";
      ...
      if (doc["soil moisture"] < soilmoisture_at_least) {
        ...
        char valvejson[100];
        serializeJson(valvedoc, valvejson);
        http.begin(client, "http://575781d3.ngrok.io/valvereply");
        http.addHeader("Content-Type", "application/json");  //Specify content-type header
        http.POST(valvejson);   //Send the request
        http.end();
